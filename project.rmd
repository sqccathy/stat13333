...
```{r}
# Saving the seeds for RNG when n is 50
...
# Finding the seed for four of 1000 node configurations corresponding roughly to the min, max, median and mean values of Rc50
SimTime = numeric(4)
SimTime[1] = which(RC[, 1] == min(RC[, 1]))
SimTime[2] = which(RC[, 1] == max(RC[, 1]))
diffmedian = abs(RC[, 1] - median(RC[, 1]))
SimTime[3] = head(which(diffmedian == min(diffmedian)), 1)
diffmean = abs(RC[,1] - mean(RC[,1]))
SimTime[4] = head(which(diffmean == min(diffmean)), 1)
```

Now that we have the seeds we need, we can then plot the network of connected points for four of 1000 node configurations corresponding roughly to the min, max, median and mean values of Rc50.
```{r}
# Finding the points between which the distances are less than or equal to Rc.
for(i in 1:4) {
  .Random.seed = seeds[[SimTime[i]]]
  nodes = genNodes(50)
  Rc = findRc(nodes)
  x = nodes[, 1]
  y = nodes[, 2]
  # Finding the points which can be connected
  distBNodes = as.matrix(dist(nodes))
