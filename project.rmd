...
repRc=numeric(3)
.Random.seed = seeds[[which(Rc == min(Rc))-1]]
repRc[1] = findRc(genNodes(100), tol = 0.001)
.Random.seed = seeds[[which(Rc == max(Rc))-1]]
repRc[2] = findRc(genNodes(100), tol = 0.001)
.Random.seed = seeds[[which(Rc == median(Rc))-1]]
...
## Plot the network
require(ggplot2)
ggplot(data = data.frame(x1, y1), mapping = aes(x = x1, y = y1))+
  geom_point()+
  geom_segment(data = data.frame(x1, y1, x2, y2), mapping = aes(x = x1, y =y1, xend = x2, yend = y2))
