...
Notice that to reproduce the simulation results, we use set.seed() to control the seed. We also give the seed used at the beginning of simulations.
```{r}
# Setting the seeds for each simulation
...
# Finding Rc for different number of nodes, each with 1000 simulations
...
## 上面跑出来挺好的~ 以下是还没做完的，明天再做啦啦啦啦
```{r}
repRc=numeric(3)
.Random.seed = seeds[[which(Rc == min(Rc))]]
repRc[1] = findRc(genNodes(100), tol = 0.001)
.Random.seed = seeds[[which(Rc == max(Rc))]]
repRc[2] = findRc(genNodes(100), tol = 0.001)
.Random.seed = seeds[[which(Rc == median(Rc))]]
repRc[3] = findRc(genNodes(100), tol = 0.001)
## Find the points between which the distances are less than or equal to the Rc found using the function findRc.

M = genNodes(100)
R = findRc(M)
ConnectMat = nodesConnect(M, R)
x = M[, 1]
y = M[, 2]
n = nrow(M)

## Find the points which can be connected

logical = as.logical(findTranMat())
logical = matrix(logical, nrow = n, ncol = n, byrow = TRUE) 
...
## Plot the network
ggplot(data = data.frame(x1, y1, x2, y2))+
